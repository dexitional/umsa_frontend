...
export async function loader({ params }){
  const data = await Service.fetchNotice(params.noticeId)
   return { data }
}

function PgAISPNotice({}: Props) {
  const { data } :any = useLoaderData();
  return (
    <main className="md:pl-10 p-3 md:p-6 space-y-3 md:space-y-10">
      <SubPageTitle title={data?.subject} page="Notice" />
            ...
            <div className="flex-1 flex flex-col space-y-6 md:space-y-5">
              <div className="flex space-x-2">
                  ...
                  <div className="space-y-3">
                    <h1 className="text-md md:text-3xl md:tracking-wide leading-5 font-semibold text-blue-950/60">{data?.subject}</h1>
                    <div className="w-full flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-2 text-zinc-400 text-lg">
                      ...
                      <div className="px-3 py-0.5 flex items-center space-x-2 border rounded">
                          ...
                          <span className="text-sm md:text-base font-medium text-blue-950/70 capitalize">All NSS Personels</span>
                      </div>
                    </div>
                  </div>
                  
              </div>
              <p className="text-gray-400 md:text-gray-600 text-sm md:text-lg">{data?.message}</p>
            </div>
         </section>
