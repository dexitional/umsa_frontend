...
function PgDricPersonnel({}: Props) {
  ...
  return (
    <main className="md:pl-10 p-3 md:p-6 space-y-3 md:space-y-10">
      <SubPageTitle title={`${data?.title} ${data?.fname} ${data?.mname && data?.mname+' ' }${data?.lname}`} page="Personnels" />
      <div className="p-3 md:p-6 border bg-slate-50/50 rounded-xl md:space-y-6 space-y-4">
         <section className="relative flex space-x-2 md:space-x-6">
            <div className="hidden md:block p-2 md:p-4 h-16 w-16 md:h-48 md:w-44 border rounded-xl shadow-lg bg-white">
              <img src={`https://cdn.ucc.edu.gh/photos/?tag=${data?.identity}` ?? Logo} className="h-14 w-14 md:h-40 md:w-40 object-cover  object-center" />
            </div>
            ...
            <div className="flex-1 flex flex-col space-y-4 md:space-y-3">
              <div className="flex space-x-2">
                  <div className="block md:hidden p-2 md:p-4 h-16 w-16 md:h-48 md:w-44 border rounded-xl shadow-lg bg-white">
                    <img src={`https://cdn.ucc.edu.gh/photos/?tag=${data?.identity}` ?? Logo} className="h-14 w-14 md:h-40 md:w-40 object-contain" />
                  </div>
                  <div className="w-full flex flex-col text-zinc-400 text-lg">
                     ...
                     <div className="space-x-2">  
                           <span className="text-xs md:text-lg tracking-wider">{data?.identity}</span>
         ...
         <section className="space-y-4">
            {/* Project History */}
            <div className="p-2 md:py-4 md:px-6 flex flex-col space-y-3 md:space-y-6 border rounded-md md:rounded-xl bg-white">
               <h1 className="py-0.5 px-2 md:px-3 w-fit text-xs md:text-base font-semibold rounded-md bg-green-950/50 text-white tracking-widest uppercase -skew-x-6">PROJECTS - ACTIVITY PERSONNEL</h1>
               <div className="md:pl-6 space-y-4">
                  ...
                  { !data?.activities.length && (<h1 className="ml-4 md:ml-0 w-full text-left text-gray-400 text-[0.65rem] md:text-xs font-medium tracking-widest uppercase italic">No Recorded projects ...</h1>)}
               </div>
             </div>
