...
export async function action({ request, params }){
  const id = params?.personelId || 0;
  const formData = await request.formData()
  let data = Object.fromEntries(formData)
  let resp;
   if(id != 0) 
     resp = await Service.updatePersonel(id,data);
   else
     resp = await Service.postPersonel(data);
  
   if(resp){
     return redirect(`/dric/personels/${resp.id}`)
   }
} 
...
export async function loader({ params }){
  ...
  const id = params?.personelId || 0;
  if(id != 0)
    data = await Service.fetchPersonel(id)
  return { data }
}

function PgDricPersonnelForm({}: Props) {
  ...
  return (
    <main className="md:pl-10 p-2 md:p-6 space-y-4 md:space-y-10">
      <SubPageTitle title={`${data?.id ? 'Edit':'Create'} Personnel`} page="Personnels" />
      <div className="p-2 md:p-6 border bg-slate-50/50 rounded-xl space-y-6">
         <section className="flex md:space-x-6">
           <div className="flex-1 flex flex-col space-y-1 md:space-y-3">
              <h1 className="text-lg md:text-2xl tracking-wide font-semibold text-blue-950/70">{data?.id ? 'Edit':'Create'} Personnel</h1>
         ...
         <Form method="post" className="grid md:grid-cols-2 gap-y-2 md:gap-y-0 md:gap-x-4">
             ...
             <div className="p-3 md:py-6 md:pb-10 md:px-6 border rounded-lg md:rounded-xl bg-white space-y-3 md:space-y-6">
               ...
               <div className="md:pl-6 space-y-4">
                  <label className="flex flex-col space-y-2">
                      <span className="text-sm md:text-base text-gray-500 font-medium">Staff Number or NSS Number</span>
                      <input arial-label="identity"  name="identity" defaultValue={data?.identity} required className="focus:ring-0 border focus:border-slate-300  border-slate-200 bg-blue-500/5 text-sm md:text-base text-gray-500 rounded-md" />
                  </label>
                  <label className="flex flex-col space-y-2">
...
export default PgDricPersonnelForm