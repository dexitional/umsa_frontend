...
export async function loader({ request }) {
  ...
  const data = await Service.fetchResits(search,page);
  console.log(data)
...
function PgAISResits({}: Props) {
  ...
  return (
    <div className="md:pl-10 p-4 md:p-6 space-y-4 md:space-y-10">
      <PageTitle title="Resits" createtext="" createlink="create" pages={data?.totalPages} setView={setView} view={view} />
      <div className="">
         { view == 'card' && (
            <div className={`grid ${data?.data?.length ? 'md:grid-cols-3':'md:grid-cols-2 justify'} gap-3 md:gap-6`}>
              { data?.data?.map((row:any) => (<ResitCardItem key={row.id} data={row} /> ))}
