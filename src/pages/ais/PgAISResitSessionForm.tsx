...
export async function action({ request, params }){
   ...
   let data = Object.fromEntries(formData)
    ...
    let resp;
    if(id != 0) 
      resp = await Service.updateResitSession(id,data);
    else
      resp = await Service.postResitSession(data);
   
    if(resp){
      return redirect(`/ais/resit-sessions`)
      //return redirect(`/ais/resit-sessions/${encodeURIComponent(resp.id)}`)
    }
} 
// Load Data of Single 
export async function loader({ params }){
   ...
   const id = params?.sessionId || 0;
   if(id != 0)
     data = await Service.fetchResitSession(id)
   return { data }
}

function PgAISResitSessionForm({}: Props) {
  
  const navigate = useNavigate()
  const { data }: any = useLoaderData();
  return (
    <main className="md:pl-10 p-2 md:p-6 space-y-4 md:space-y-10">
      <SubPageTitle title={`${data?.id ? 'Edit':'Create'} Resit Session`} page="Resit Session" />
      <div className="p-2 md:p-6 border bg-slate-50/50 rounded-xl space-y-6">
         <section className="flex md:space-x-6">
           <div className="flex-1 flex flex-col space-y-1 md:space-y-3">
              <h1 className="text-lg md:text-2xl tracking-wide font-semibold text-primary/70">{data?.id ? 'Edit':'Create'} Resit Session</h1>
             ...
             <div className="p-3 md:py-6 md:pb-10 md:px-6 w-full border rounded-lg md:rounded-xl bg-white space-y-3 md:space-y-6">
                <div className="md:pl-6 space-y-4">
                  ...
                  <label className="flex flex-col space-y-2">
                      ...
                      <input arial-label="end" type="date" name="end" defaultValue={moment(data?.end).format("YYYY-MM-DD")} required className="focus:ring-0 border focus:border-slate-300  border-primary-dark/10 bg-primary-dark/5 text-sm md:text-base text-gray-500 rounded-md" />
                  </label>
                  <label className="flex flex-col space-y-2">
                      <span className="text-sm md:text-base text-gray-500 font-medium">Session Period</span>
                      <input arial-label="period" type="date" name="period" defaultValue={moment(data?.period).format("YYYY-MM-DD")} required className="focus:ring-0 border focus:border-slate-300  border-primary-dark/10 bg-primary-dark/5 text-sm md:text-base text-gray-500 rounded-md" />
                  </label>
...
export default PgAISResitSessionForm