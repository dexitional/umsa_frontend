...
import UnitListView from '../../components/ais/UnitListView'
type Props = {}
...
function PgAISUnits({}: Props) {
  ...
  return (
    <div className="md:pl-10 p-4 md:p-6 space-y-4 md:space-y-10">
      <PageTitle title="Units" createtext="New" createlink="create" pages={totalPages} setView={setView} view={view} />
      <div className="">
         { view == 'card' && (
            <div className="grid md:grid-cols-3 gap-3 md:gap-6">
              { data && data?.map((row:any) => (<UnitCardItem key={row.id} data={row} /> ))}
