...
export async function action({ request, params }){
   ...
   let data = Object.fromEntries(formData)
    ...
    let resp;
    if(id != 0) 
      resp = await Service.updateCurriculum(id,data);
    else
      resp = await Service.postCurriculum(data);
...
export async function loader({ params }){
   ...
   const id = params?.curriculumId || 0;
   if(id != 0)
     data = await Service.fetchCurriculum(id)
   return { data,departments,programs,courses }
}

function PgAISSheetForm({}: Props) {
  
  const navigate = useNavigate()
  const { data,departments,programs,courses }: any = useLoaderData();
  return (
    <main className="md:pl-10 p-2 md:p-6 space-y-4 md:space-y-10">
      <SubPageTitle title={`${data?.id ? 'Edit':'Create'} Curriculum`} page="Curriculums" />
      <div className="p-2 md:p-6 border bg-slate-50/50 rounded-xl space-y-6">
         <section className="flex md:space-x-6">
           <div className="flex-1 flex flex-col space-y-1 md:space-y-3">
              <h1 className="text-lg md:text-2xl tracking-wide font-semibold text-primary/70">{data?.id ? 'Edit':'Create'} Curriculum</h1>
         ...
         <Form method="post" className="grid md:grid-cols-2 gap-y-2 md:gap-y-0 md:gap-x-4">
             ...
             <div className="p-3 md:py-6 md:pb-10 md:px-6 border rounded-lg md:rounded-xl bg-white space-y-3 md:space-y-6">
               ...
               <div className="md:pl-6 space-y-4">
                  ...
                  <label className="flex flex-col space-y-2">
                      <span className="text-sm md:text-base text-gray-500 font-medium">Category</span>
             ...
             <div className="p-3 md:py-6 md:pb-10 md:px-6 w-full border rounded-lg md:rounded-xl bg-white space-y-3 md:space-y-6">
                <div className="md:pl-6 space-y-4">
                  ...
                  <label className="flex flex-col space-y-2">
                      <span className="text-sm md:text-base text-gray-500 font-medium">Lock on Registration <sup className="text-primary/80"><em>** Electives only</em></sup></span>
                      <select arial-label="lock" name="lock" defaultValue={Number(data?.lock)} required className="focus:ring-0 border focus:border-slate-300  border-primary-dark/10 bg-primary-dark/5 text-sm md:text-base text-gray-500 rounded-md">
                        <option selected disabled>-- Choose --</option>
