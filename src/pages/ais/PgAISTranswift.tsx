         ...
         <section className="relative flex space-x-2 md:space-x-6">
            ...
            <div className="flex-1 flex flex-col space-y-4 md:space-y-3">
              <div className="flex space-x-2">
                  ...
                  <h1 className="text-md md:text-3xl md:tracking-wide leading-5 font-semibold text-primary/70">{data?.transact?.transtype?.title}</h1>
              </div>
              <div className="w-full flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-2 text-zinc-400 text-lg">
                <div className="flex items-center space-x-2"> 
                    ...
                    <span className="tracking-wider text-xs md:text-base capitalize">{data?.studentId }</span>
                    <div className="w-1.5 h-1.5 rounded-full bg-slate-400"></div>
                 </div>
                 <div className="flex items-center space-x-2">
                    <span className="px-3 py-0.5 bg-white border border-primary/40 text-sm text-gray-500 relative">{data?.student.fname} {data?.student.mname ? data?.student.mname+' ':''}{data?.student.fname}</span>
                </div>
              </div>
            </div>
         </section>
