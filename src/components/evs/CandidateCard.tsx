            ...
            <div className="flex-1 flex flex-col space-y-1.5">
                <div className={ showResult ? `block`: `hidden`}>
                    <div className="my-2 px-1 py-1 rounded border-2 border-blue-950/20 text-sm md:text-base text-blue-950/80 md:text-blue-950/80 bg-white font-bold md:font-bold space-x-2">
                        ...
                        <span className="p-0.5 px-1.5 text-sm md:text-base italic rounded border bg-red-100 tracking-wide">{ (data?.votes/vtotal * 100).toFixed(1) }%</span>
                    </div>
                    { winner && <h3 className="px-2 w-fit rounded bg-green-600 text-xs md:text-sm text-white font-bold italic tracking-widest">WINNER</h3>}
                </div>
                <h3 className="text-xs md:text-base text-gray-600 font-bold italic">{data?.teaser || data?.tag}</h3>
                <div className="w-full flex items-center justify-between">
                <h3 className="w-fit text-3xl text-gray-500 font-bold tracking-widest">#{data?.orderNo}</h3>
