  ...
  return (
    <label htmlFor={`${action}`} className=" cursor-pointer overflow-hidden relative px-3 p-2 w-full h-fit border border-slate-300 rounded flex space-x-3 items-start">
        <div className="relative h-10 w-8 md:h-10 md:w-10 rounded-full bg-slate-200 flex items-center justify-center">
            ...
            <input className="h-6 w-6 rounded-full focus:outline-none focus:ring-0  checked:bg-blue-950/80 checked:text-blue-950/80" id={action} type="checkbox" defaultChecked={data && data[action]} name={action} onChange={onChange} />
        </div>
        ...
        <button className={`px-2 py-1 absolute -right-5 top-4 rotate-90  bg-slate-100 ${tag == 'ADMIN' ? 'text-red-500/80':'text-blue-950/80'} text-xs font-bold`}>{tag}</button>
