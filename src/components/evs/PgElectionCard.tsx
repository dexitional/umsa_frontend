...
function PgElectionCard({ data }: Props) {
  ...
  return (
    ...
    <div className="px-4 py-2 md:py-1 md:px-3 w-full md:w-64 rounded bg-[#f1f1f1]/30 shadow-inner shadow-gray-500/30 space-y-6">
        <div className="text-center text-base md:text-lg text-blue-950 font-bold font-mono">{data?.title}</div>
    </div>
    <div className="p-4 md:py-6 md:px-3 w-full md:w-64 h-fit rounded bg-[#f1f1f1]/30 shadow-inner shadow-gray-500/30 space-y-6">
        ...
        <div className="py-2 px-2 rounded shadow-inner shadow-gray-500/20 bg-white gap-y-2 grid grid-cols-2 place-content-center overflow-hidden">
            ...
            <h3 className='py-1 text-lg md:text-lg text-center border border-blue-950/80 text-blue-950 font-bold'>{data?.voterData?.length}</h3>
            <h3 className="py-1 bg-blue-950/80 font-bold text-sm text-white text-center flex items-center justify-center">TURNOUT</h3>
            <h3 className='py-1 text-lg md:text-lg text-center border border-blue-950/80 text-blue-950 font-bold'>{data?.turnout}</h3>
            <h3 className="py-1 bg-blue-950/80 font-bold text-sm text-white text-center flex items-center justify-center">STATUS</h3>
            <h3 className='py-1 text-sm md:text-sm text-center border border-blue-950/80 text-blue-950 font-bold'>{data?.action}</h3>
            <div className="p-2 col-span-2 rounded font-roboto border border-blue-950/80 space-y-2">
                <div className="w-full flex items-center justify-between"><span className="py-0.5 px-1.5 rounded bg-blue-950/80 text-white text-xs tracking-widest font-bold inline-block uppercase">Start</span> <span className="font-extrabold text-blue-950 tracking-wide text-sm inline-block align-right uppercase">{data.startAt && moment(data.startAt).format("MMM DD, YYYY HH:mm")}</span></div>
                <div className="w-full flex items-center justify-between"><span className="py-0.5 px-1.5 rounded bg-blue-950/80 text-white text-xs tracking-widest font-bold inline-block uppercase">End</span> <span className="font-extrabold text-blue-950 tracking-wide text-sm inline-block align-right uppercase">{data.endAt && moment(data.endAt).format("MMM DD, YYYY HH:mm")}</span></div>
            </div>
        </div>
