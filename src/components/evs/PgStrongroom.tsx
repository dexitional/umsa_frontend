...
function PgStrongroom() {
  ...
  return (
    <div className="py-3 px-3 flex-1 h-full rounded bg-[#f1f1f1]/30 shadow-inner shadow-gray-500/30 space-y-3 md:space-y-6">
        <h1 className="px-4 py-2.5 text-base md:text-xl rounded bg-blue-950/80 font-semibold font-noto text-white ">STRONGROOM MONITOR</h1>
        <div className="py-2     md:py-4 px-2 rounded shadow-inner shadow-gray-500/20 bg-white space-y-4">
            ...
            <div className="pt-2 flex items-center justify-center space-x-2">
              { chartData?.map((row:any, i:number) => 
                <div className={`h-4 w-4 md:h-6 md:w-6 rounded-full border-4 ${pageview == i ? 'bg-blue-950/80':'bg-slate-100'} `}></div>
              )}
            </div>
            { chartData?.map((row:any, i:number) => 
               pageview == i ? (
