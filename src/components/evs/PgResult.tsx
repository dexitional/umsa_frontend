  ...
  return (
    <div className="py-3 px-3 flex-1 h-full rounded bg-[#f1f1f1]/30 shadow-inner shadow-gray-500/30 space-y-6">
        <h1 className="px-4 py-2.5 flex items-center justify-between text-lg md:text-xl rounded bg-blue-950/80 font-semibold text-white">
          ...
          <button className="p-0.5 px-2 rounded bg-purple-50 text-base text-blue-950 font-bold tracking-wider">PRINT</button>
        </h1>
        <div className="py-4 px-2 rounded shadow-inner shadow-gray-500/20 bg-white space-y-4">
            <div className="mx-2 px-10 py-3 rounded flex flex-col md:flex-row items-center justify-center space-x-4 text-sm md:text-lg text-center text-blue-950 font-bold tracking-widest bg-slate-100/70">
                  ...
                  <div key={row.id} className="px-2 py-2 flex-1 bg-white rounded space-y-2">
                    ...
                    <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-2 place-content-start overflow-y-scroll">
                        { row?.candidates?.map((r:any, i: number) => {
                           if( i == 0 ) return <CandidateCard key={r?.id} data={r} vtotal={data?.electors?.length} showResult winner />
                           return (<CandidateCard key={r?.id} data={r} vtotal={data?.electors?.length} showResult />)}
                         )}
                    </div>
                  </div>
                ))}
