  ...
  return (
    ...
    <div className="space-y-1 font-roboto">
        ...
        <div className="flex items-center space-x-4">
            ...
            <span className="px-2 py-0 bg-green-50 rounded border text-sm text-gray-500">Graduation Starts: &nbsp;&nbsp;&nbsp;<b>{moment(data.start).format("MMM DD, YY")}</b></span>
        </div>
        }
        { data.end && 
        <div className="flex items-center space-x-4">
            <IoIosTime className="h-4 w-5 text-primary/70" />
            <span className="px-2 py-0 bg-green-50 rounded border text-sm text-gray-500">Graduation Ends:&nbsp;&nbsp;&nbsp;<b>{moment(data.end).format("MMM DD, YY")}</b></span>
        </div>
        } 
        <div className="flex items-center space-x-4">
            <IoIosTime className="h-4 w-5 text-primary/70" />
            <span className="px-2 py-0 bg-green-50 rounded border text-sm text-gray-500">Graduants:&nbsp;<b>{data?._count?.graduate || 0}</b></span>
        </div>
    ...
    <div className="flex flex-col space-y-1">
        ...
        <div className="px-3 py-2 opacity-80 md:opacity-100 flex rounded-md border bg-white items-center md:justify-between space-x-2 group">
          <Link to={`${encodeURIComponent(data?.id)}/graduants`} className="py-0.5 px-2 rounded flex items-center space-x-1.5 bg-primary/60">
