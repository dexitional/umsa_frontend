...
function HRSNavItem({ title, url = '/', Icon }: Props) {
 
  return (
    <NavLink 
    to={url} 
    className={({ isActive, isPending }) => isActive ? `px-4 py-2 flex items-center space-x-3 bg-red-950/70 rounded-lg`: `px-4 py-2 flex items-center space-x-2 hover:bg-red-950/70 hover:rounded-lg group`}
    children={({ isActive }) => {
      return(
        <>
