  ...
  return (
    <div className="p-4 md:p-6 min-h-max border rounded-xl bg-slate-50/50 hover:bg-slate-100 space-y-4 group">
        ...
        <div className="flex items-center space-x-3">
            <div className="text-sm md:text-base text-gray-400 capitalize">{data?.country?.longName}</div>
            <div className="py-0.5 px-2 text-sm rounded bg-blue-950/60 text-white font-bold">{data?.projects?.length} Projects</div>
        </div>
        <div className="space-y-1">
            ...
            <div className="flex items-center space-x-4">
                ...
                <span className="text-sm text-gray-500 italic">{data?.publicPage}</span>
            </div>
        </div>
